...
// 因此 HomeSpan 抽象了所有这些细节。由于 C++ 遵循严格的变量类型，因此这是通过使用模板方法来实现的。每个 Characteristic 都支持以下两种方法：
...
// 访问存储为 2 字节 int 的值。但是使用 getVal<int>() 来访问 HAP 坚持以浮点数存储的 Characteristic 的值是完全可以的，即使它的范围严格在 0 到 100 之间，步长为 1。
// 了解范围和步长大小是确定是否可以将其作为 <int> 甚至 <uint8_t> 访问所需的全部知识。

// 由于大多数 Characteristic 值都可以正确地转换为 int，因此如果未指定模板参数，getVal 和 getNewVal 都默认为 <int>。如上所示，我们通过简单地调用 power->getNewVal() 
// 检索了 HomeKit 为名为“power”的 On Characteristic 请求的新值。由于未指定模板参数，getNewVal() 将返回一个 int。并且由于 On Characteristic 本身存储为布尔值，
...
// 最后，还有一个用于 Characteristics 的附加方法，上面没有使用，但将在后面的例子中使用：updated()。此方法返回一个布尔值，指示 HomeKit 是否已请求更新 Characteristic，
// 这意味着 getNewVal() 将包含它想要为该 Characteristic 设置的新值。对于只有一个 Characteristic 的服务，如上所述，我们不需要询问是否使用 power->updated() 更新了“power”，
