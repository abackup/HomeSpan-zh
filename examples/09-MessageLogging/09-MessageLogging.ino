...
void setup() {
  ...
  // 每当设备连接到计算机时，HomeSpan 都会向 Arduino IDE 的串行监视器发送各种消息。消息输出可以通过常用的 Serial.print() 
  ...
  //  LOG0() 消息将始终输出到 Arduino 串行监视器。仅当日志级别设置为 1 或 2 时，才会输出 LOG1() 消息。
