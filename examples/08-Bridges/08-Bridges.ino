...
void setup() {
  ...
  // 1. 回想一下示例 7，使用 Characteristic::Name() 更改附件图块的默认名称对定义的第一个附件不起作用。Home App 始终将第一个附件图块的默认名称显示为 homeSpan.begin() 
  ...
  // 2. 配置为桥接器的设备出现在 Home App 的主设置页面下，该页面显示所有集线器和桥接器。
  ...
  // 下面我们将 Category::Lighting 替换为 Category::Bridges。这会更改与 Home App 配对时显示的设备图标，
  // 但不会更改 Accessory Tiles 的图标。您可以选择任何您喜欢的类别。
  ...
  new SpanAccessory();                            // 第二个附件与示例 7 中的第一个附件相同，不同之处在于 Characteristic::Name() 现在执行一些操作
    new Service::AccessoryInformation();
      new Characteristic::Identify();            
      new Characteristic::Name("Simple LED");     // 请注意，与示例 7 不同，Name() 的这种用法现在由 Home App 使用，因为它不是第一个配件（上面的 Bridge 是第一个）
    new DEV_LED(16);
