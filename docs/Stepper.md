<!-- 原文时间：2024.2.18 ，翻译时间：2024.5.7，校对时间：2024.5.30 -->
...
*  `StepperControl *setAccel(float accelSize, float accelSteps)`
    ...
    * 步骤之间的总延迟（*stepsRemaining* 当不为零时）由以下公式给出：$$totalDelay = msDelay \times (1 + accelSize \times (e^{\frac{-\mid nSteps-stepsRemaining \mid}{accelSteps}} + e^{\frac{-(\mid stepsRemaining \mid - 1)}{accelSteps}}))$$
