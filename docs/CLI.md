...
你可以使用 `SpanUserCommand()` 使用自定义函数扩展 HomeSpan CLI。此类允许你为任何自定义函数分配一个单字符名称，当你在 CLI 中键入 "@" 符号后跟单字符名称时将调用该函数。例如，如果你将字符 "K" 分配给自定义函数，则可以在 CLI 中键入 "@K" 来调用它。这允许你使用任何单字符名称，即使 HomeSpan 已将该字符用于其内置命令。`SpanUserCommand` 类还允许你包含函数的简短文本描述，当你键入 "？" 时，该描述将添加到命令菜单中。进入 CLI。有关完整详细信息，请参阅 [HomeSpan API 参考](Reference.md#spanusercommand)。
