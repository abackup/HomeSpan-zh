原文时间：2024.2.18 ，翻译时间：2024.5.13

# 可寻址 RGB LED

HomeSpan 包括两个专用类，可轻松控制“可寻址” RGB LED。**Pixel()** 类用于仅需要单个“数据”控制线的 RGB 和 RGBW LED，例如此 8 像素[NeoPixel RGB 灯条](https://www.adafruit.com/product/1426)或单灯珠 [NeoPixel RGBW LED](https://www.adafruit.com/product/2759)。**Dot()** 类用于需要两条控制线（“数据”和“时钟”）的 RGB LED，例如这个 144 像素 [DotStar RGB 灯带](https://www.adafruit.com/product/2241)或这个 60 像素 [RGB LED 灯带](https://www.sparkfun.com/products/14015)。 
...
创建此**类**的实例可将指定的 *pin* 配置为输出适合控制具有任意数量的单线、可寻址 RGB 或 RGBW LED 设备的波形信号。像素。此类设备通常包含 SK6812 或 WS2812 LED。参数及其默认值（如果未指定）如下：
  ...
  * *isRGBW* - 对于包含 4 色（红/绿/蓝/白）LED 的 RGBW 设备，设置为 *true*；对于更典型的 3 色 RGB 设备，设置为 *false*。如果未指定，则默认为 *false*。请注意，如果你使用 RGBW 设备，即使你不打算使用白色 LED，也必须将 *isRGBW* 标志设置为 *true*
...
创建此**类**的实例将指定的引脚配置为输出适合控制具有任意数量像素的两线、可寻址 RGB LED 设备的波形信号。此类设备通常包含 SK9822 或 APA102 LED，或嵌入式 WS2801 驱动器芯片。参数及其默认值（如果未指定）如下：
...
一个完整的示例展示了如何在 HomeSpan 草图中使用 Pixel 库来控制 RGB Pixel 设备、RGBW Pixel 设备和 RGB DotStar 设备（所有这些都来自 iPhone 上的家庭应用程序），可以在 Arduino IDE 中找到[*文件→示例→ HomeSpan →其他示例→像素*](../examples/Other%20Examples/Pixel)。

如需更完整地展示 Pixel 库，请查看 [HomeSpan 项目](https://github.com/topics/homespan)上的[节日灯](https://github.com/HomeSpan/HolidayLights)。此草图演示了如何使用 Pixel 库通过 60 像素 RGBW 条生成各种特殊效果。该草图还展示了使用 HomeSpan 的[自定义特征宏](Reference.md#custom-characteristics-and-custom-services-macros)来实现在 Eve for HomeKit 应用程序中使用的特效“选择器”按钮。
